"""
Example script for streaming orders using the TradeStation API.

This script demonstrates how to connect to the TradeStation API,
authenticate, and subscribe to the order stream for specified accounts.
It prints out order update events as they are received.

Usage:
- Ensure you have a valid .env file with your CLIENT_ID, REFRESH_TOKEN,
  and ENVIRONMENT (simulation or live).
- Run the script from the project root directory:
  poetry run python examples/Brokerage/stream_orders.py
"""
...
async def main():
    """
    Main function to stream orders.
    """
    ...
    # Get credentials and environment from environment variables
    # Client initialization handles this internally now
    ...
    try:
        ...
        print(f"\nStarting order stream for account(s): {account_ids_str}...")
        print("Press Ctrl+C to stop streaming\n")
        stream_reader = await ts_client.brokerage.stream_orders(account_ids_str)
        ...
        while running:
            try:
                ...
                try:
                    ...
                    if "OrderID" in data and "Status" in data:  # Likely an order update
                        order_data = StreamOrderResponseData.model_validate(data)
                        print(f"Received Order Update: {order_data.model_dump_json(indent=2)}")
                        ...
                        print(
                            "--- Initial Order Snapshot Complete --- Real-time updates follow ---"
                        )
