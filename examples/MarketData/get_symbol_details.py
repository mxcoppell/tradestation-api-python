...
async def main():
    ...
    try:
        ...
        for symbol in details.Symbols:
            ...
            print(
                f"Price Format: {symbol.PriceFormat.Format} ({symbol.PriceFormat.Decimals or 'N/A'} decimals)"
            )
        ...
        if stock.PriceFormat.Format == "Decimal":
            ...
            if stock.PriceFormat.Fraction and stock.PriceFormat.SubFraction:
                ...
                sub_fraction_value = round(
                    ((fraction - (main_fraction / denominator)) * sub_fraction * denominator)
                    / (sub_fraction / 10)
                )
