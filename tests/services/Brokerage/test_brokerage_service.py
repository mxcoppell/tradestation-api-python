...
from src.ts_types.brokerage import HistoricalOrdersById
...
class TestGetHistoricalOrdersByOrderID:
    ...
    @pytest.mark.asyncio
    async def test_get_historical_orders_by_order_id_success(self, brokerage_service, http_client_mock):
        ...
        result = await brokerage_service.get_historical_orders_by_order_id(account_ids, order_ids, since)
    ...
    async def test_get_historical_orders_by_order_id_date_validation(self, brokerage_service):
        ...
        with pytest.raises(ValueError, match="Date range cannot exceed 90 days"):
            await brokerage_service.get_historical_orders_by_order_id("123456789", "286234131", old_date)

    @pytest.mark.asyncio
    async def test_get_historical_orders_by_order_id_date_formats(self, brokerage_service, http_client_mock):
        ...
        for date_str in valid_dates:
            ...
            # Call the method
            await brokerage_service.get_historical_orders_by_order_id("123456789", "286234131", date_str)
    ...
    async def test_get_historical_orders_by_order_id_invalid_date_format(self, brokerage_service):
        ...
        with pytest.raises(ValueError, match="Invalid date format"):
            await brokerage_service.get_historical_orders_by_order_id("123456789", "286234131", "invalid-date")

    @pytest.mark.asyncio
    async def test_get_historical_orders_by_order_id_with_errors(self, brokerage_service, http_client_mock):
        ...
        # Call the method with a recent date (within 90 days)
        since = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d")
        result = await brokerage_service.get_historical_orders_by_order_id("123456789", "286234131", since)
