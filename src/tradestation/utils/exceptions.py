...
class TradeStationServerError(TradeStationAPIError):
    ...
    def __init__(
        self, 
        message: str = "Server error occurred. Please try again later.",
        **kwargs
    ):
        super().__init__(message, **kwargs)
...
class TradeStationStreamError(TradeStationAPIError):
    ...
    def __init__(
        self, 
        message: str = "Error in streaming connection.",
        **kwargs
    ):
        super().__init__(message, **kwargs)
...
def map_http_error(status_code: int, response_data: Optional[Dict[str, Any]] = None) -> TradeStationAPIError:
    """
...
def handle_request_exception(exc: Exception) -> TradeStationAPIError:
    ...
    if isinstance(exc, aiohttp.ClientResponseError):
        ...
        return TradeStationNetworkError(
            message="Server disconnected unexpectedly. Please try again.",
            original_error=exc
        )
    elif isinstance(exc, aiohttp.ClientPayloadError):
        return TradeStationAPIError(
            message="Error processing server response payload.",
            original_error=exc
        )
